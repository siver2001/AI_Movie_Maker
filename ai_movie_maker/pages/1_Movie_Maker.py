...
# --- PATH SETUP ---
# Ensure project root is in sys.path so we can import 'ai_movie_maker' package
# Current file: .../ai_movie_maker/pages/1_Movie_Maker.py
# Root needed: .../ (containing ai_movie_maker folder)
file_dir = os.path.dirname(os.path.abspath(__file__)) # pages
parent_dir = os.path.dirname(file_dir) # ai_movie_maker
...
from ai_movie_maker.services.audio import generate_audio_sync, VOICE_MAP
from ai_movie_maker.services.video_ai import generate_video_clip

# --- CONFIG LOADING ---
@st.cache_data
def load_presets():
    config_path = os.path.join(parent_dir, 'config', 'presets.json')
...
try:
    ...
    st.error(f"Failed to load user presets: {e}")
    PRESETS = {
        "visual_styles": {}, 
        "brand_voices": {}, 
...
# --- PAGE CONFIG ---
st.set_page_config(page_title="Movie Maker", layout="wide", page_icon="🎬")
...
if 'detected_models' in st.session_state and st.session_state['detected_models']:
    ...
    # Ensure options are list of strings
    det_models = list(st.session_state['detected_models'])
    selected_model_auto = st.sidebar.selectbox("Detected Models", det_models, key="auto_model_select")
            ...
            try:
                ...
                for m in models:
                    ...
                    if hasattr(m, 'name') and "gemini" in m.name.lower():
                        valid_models.append(str(m.name.split("/")[-1]))
                    elif isinstance(m, str) and "gemini" in m.lower():
                        # In case it returns strings
                        valid_models.append(str(m.split("/")[-1]))
...
vis_options = ["Custom"] + list(PRESETS["visual_styles"].keys())
visual_theme_key = st.sidebar.selectbox("Visual Theme Preset", vis_options)
    ...
    if schema_version == "v3.2 (Ad-Ready)":
        ...
        if enable_campaign:
            with st.expander("Campaign Details", expanded=True):
                ...
                c_platform = st.selectbox("Platform", list(PRESETS["platforms"]))
                c_type = st.selectbox("Campaign Type", list(PRESETS["campaign_types"]))
                ...
                voice_keys = list(PRESETS["brand_voices"].keys())
                c_voice = st.selectbox("Brand Voice", voice_keys)
                            ...
                            if not os.path.exists(audio_path):
                                ...
                                with st.spinner("Rendering video..."):
                                    try:
                                        ...
                                        st.error(f"⚠️ Library Update Required. Please RESTART the terminal/app.")
                                        st.stop()
             ...
             if all_ready:
                 with st.spinner("Assembling Full Movie..."):
                     try:
                         ...
                         st.error(f"⚠️ Library Update Required.")
                         st.stop()
